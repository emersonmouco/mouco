programa {
   cadeia nome, senha, a
    ...
    real saldo = 1500, extrato, deposito, limite = 500.00,saquei, opcao, contador = 0, acrescimo
  funcao inicio() {
  ...
  funcao saque(){
    ...
    senao se(saquei > saldo){
      escreva("\nVocê não pode sacar o que não tem!")
      escreva("\nSeu saldo atual é de: ",saldo)
      escreva("\nDeseja utilizar o limite de seu cartão e acrescentar ",limite,"R$ ao seu saldo atual?\nSIM - 1\nNÃO - 2\n")
      leia(opcao)
      escolha(opcao){
        caso(1): 
        saldo += limite
        ...
        para(contador; contador < 1; contador++){
          se(contador > 1){
            escreva("\nVocê já usou seu limite!")
             pare
          }
        }
        escreva("\nSeu saldo foi atualizado para: ",saldo)
